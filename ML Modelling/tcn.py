...
for key, value in data_dict.items():
    print(f"Training model for category {key}")
    model = memotcn_model(input_shape, 1)
    model.fit(X_train[key], y_train[key], epochs=50, batch_size=32, verbose=1)
    models[key] = model

    model_file_name = f"memotcn_model_{key}.h5"
